...
This repository ships **deep-inquiry** as one canonical Agent Skill and as a skills-only
...
Invoke `$deep-inquiry` in Codex or `/deep-inquiry` in Claude Code. The host chooses when to
...
and `/plugin install`. Invoke the plugin skill as `/deep-inquiry:deep-inquiry`.
...
marketplace and install **deep-inquiry**. The CLI commands register a catalog; they are
...
```text
.agents/skills/deep-inquiry/       Canonical runtime bundle (standalone installation)
